...
accuracy_knn = train_and_evaluate_knn(X_train_np, y_train, X_test_np, y_test)
...
accuracy_lr = train_and_evaluate_lr(X_train_np, y_train, X_test_np, y_test)
...
accuracy_rf = train_and_evaluate_rf(X_train_encoded, y_train, X_test_encoded, y_test)
...
accuracy = train_and_evaluate_svm(X_train_tensor, y_train_tensor, X_test_tensor, y_test_tensor)
...
class CNNModel(nn.Module):
    def __init__(self, num_classes, num_features):
        ...
        #     nn.Conv2d(1, 32, kernel_size=1, padding=1),
        #     nn.BatchNorm2d(32),
        ...
        self.fc1 = nn.Linear(1920, 1024)  # 假设池化后的高度为56
...
for epoch in range(num_epochs):
    ...
    print(f'Epoch [{epoch + 1}/{num_epochs}], Validation Accuracy: {current_accuracy:.2f}%')

    # 检查是否是最佳准确率
    if current_accuracy >= best_accuracy:
        best_accuracy = current_accuracy
        torch.save(model.state_dict(),'acc_model.pth')  # 保存最佳模型的状态
